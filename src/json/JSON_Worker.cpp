         ...
         while (brac){\
            switch (*(++p)){\
                ...
                case left:\
                   ...
                   break;\
                QUOTECASE()\
                NULLCASE(TEXT("Null terminator inside of a square bracket"))\
            }\
         }\
         break;}\
    case right:\
...
json_char * Worker::RemoveWhiteSpace(const json_string & value_t){
    ...
    while(*p){
	   switch(*p){
          ...
          case TEXT('/'):  //a C comment
                if (*(++p) == TEXT('*')){  //a multiline comment
                    while ((*(++p) != TEXT('*')) || (*(p + 1) != TEXT('/'))){
					   libJSON_ASSERT(*p, TEXT("Null terminator inside of a multiline quote"));
				    }
				    ++p;
				    break;
                }
                //Should be a single line C comment, so let it fall through to use the bash comment stripper
                libJSON_ASSERT(*p == TEXT('/'), TEXT("stray / character, not quoted, or a comment"));
		  ...
		  default:
			 libJSON_ASSERT((unsigned json_char)*p >= 32, TEXT("Invalid JSON character detected (lo)"));
			 libJSON_ASSERT((unsigned json_char)*p <= 126, TEXT("Invalid JSON character detected (hi)"));
			 *runner++ = *p;
			 break;
	   }
	   ++p;
...
json_char Worker::FromOctal(const json_char * & str){
...
//Create a childnode
void NewNode(JSONNode * parent, const json_string & name, const json_string & value){
    JSONNode * node = new JSONNode(name, value);    //create the child node
    libJSON_ASSERT(node, TEXT("Couldn't create new node, out of memory"));
    parent -> Children.push_back(node);	    //attach it to the parent node
...
void Worker::DoArray(JSONNode * parent, const json_string & value_t){
    ...
    */
    ...
    libJSON_ASSERT(value_t[0] == TEXT('['), TEXT("DoArray is not an array"));
...
void Worker::DoNode(JSONNode * parent, const json_string & value_t){
    ...
    */
    ...
    libJSON_ASSERT(value_t[0] == TEXT('{'), TEXT("DoNode is not an node"));
    const size_t len = value_t.length();
